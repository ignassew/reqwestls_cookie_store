...
//!   reqwestplus_cookie_store::CookieStore::load_json(file).unwrap()
//! };
//! let cookie_store = reqwestplus_cookie_store::CookieStoreMutex::new(cookie_store);
...
//! }
//!
//! // Build a `reqwestplus` Client, providing the deserialized store
...
use reqwestplus::header::HeaderValue;
use url;
...
impl reqwestplus::cookie::CookieStore for CookieStoreMutex {
...
impl reqwestplus::cookie::CookieStore for CookieStoreRwLock {
    fn set_cookies(&self, cookie_headers: &mut dyn Iterator<Item = &HeaderValue>, url: &url::Url) {
